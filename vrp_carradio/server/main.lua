...
AddEventHandler("vrp_carradio:ChangeVolume", function(vol, nome)
    ...
    if somafter and rangeafter then
        TriggerClientEvent("vrp_carradio:ChangeVolume",-1,somafter,rangeafter, nome)
    end
end)

RegisterNetEvent("vrp_carradio:ChangeLoop")
AddEventHandler("vrp_carradio:ChangeLoop", function(nome,tip)
	local loopstate
	for i = 1, #Config.Zones do
		local v = Config.Zones[i]
		if nome == v.name then
	...
	if loopstate ~= nil then
		TriggerClientEvent("vrp_carradio:ChangeLoop",-1,loopstate, nome)
	end
end)

RegisterNetEvent("vrp_carradio:ChangeState")
AddEventHandler("vrp_carradio:ChangeState", function(type, nome)
	for i = 1, #Config.Zones do
		local v = Config.Zones[i]
		if nome == v.name then
			v.isplaying = type
	...
	TriggerClientEvent("vrp_carradio:ChangeState",-1,type, nome)
end)
...
AddEventHandler("vrp_carradio:ChangePosition", function(quanti, nome)
	...
	TriggerClientEvent("vrp_carradio:ChangePosition",-1,quanti, nome)
end)
...
AddEventHandler("vrp_carradio:ModifyURL", function(data)
	...
	if zena then
		TriggerClientEvent("vrp_carradio:ModifyURL",-1,zena)
...
AddEventHandler("vrp_carradio:AddVehicle", function(vehdata)
    local Data = {}
    ...
    TriggerClientEvent('vrp_carradio:AddVehicle', math.floor(-1), Config.Zones[#Config.Zones])
end)

RegisterNetEvent('vrp_carradio:GetDate')
AddEventHandler('vrp_carradio:GetDate', function()
    TriggerClientEvent('vrp_carradio:SendData', math.floor(-1), Config.Zones)
end)