const gunoier = new Vue({
    el: ".gunoier-container",
    data: {
        active: false,
        userSkill: 1,
        selectedSkill: 1,
    ...
    methods: {
        ...
        build(data) {
            ...
            $(".gunoier-container").fadeIn();
            $(".jobs-container").fadeIn();
        },
        destroy() {
            if (!this.active) return;
            this.active = false;
            $(".gunoier-container").fadeOut();
            $(".jobs-container").fadeOut();
            this.post('jobExit', {exit: true})
        },
        startgunoier: function() {
            this.post('startJob', {job: 'Gunoier', skill: this.selectedSkill});
            this.destroy();
        },
        StopJob() {
            this.post('quitJob', {job: 'Gunoier'});
            this.destroy();
        },
