const scafandru = new Vue({
    el: ".scafandru-container",
    data: {
        active: false,
        userSkill: 1,
        selectedSkill: 1,
    ...
    methods: {
        ...
        build(data) {
            ...
            $(".scafandru-container").fadeIn();
            $(".jobs-container").fadeIn();
        },
        destroy() {
            if (!this.active) return;
            this.active = false;
            $(".scafandru-container").fadeOut();
            $(".jobs-container").fadeOut();
            this.post('jobExit', {exit: true})
        },
        startscafandru: function() {
            this.post('startJob', {job: 'Scafandru', skill: this.selectedSkill});
            this.destroy();
        },
        StopJob() {
            this.post('quitJob', {job: 'Scafandru'});
            this.destroy();
        },
