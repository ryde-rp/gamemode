const farmer = new Vue({
    ...
    methods: {
        ...
        build(data) {
            ...
            $(".farmer-container").fadeIn();
            $(".jobs-container").fadeIn();
        },
        destroy() {
            if (!this.active) return;
            this.active = false;
            $(".farmer-container").fadeOut();
            $(".jobs-container").fadeOut();
            this.post('jobExit', {exit: true})
        },
        startfarmer: function() {
            this.post('startJob', {job: 'Farmer', skill: this.selectedSkill});
            this.destroy();
        },
        StopJob() {
            this.post('quitJob', {job: 'Farmer'});
            this.destroy();
        },
...
const farmerChoice = new Vue({
    ...
    methods: {
        ...
        onButton(res){
            this.active = false;
            $(".fairplay-farmjob").fadeOut();
            
            this.post('farmerExit', {response: res})
        },
        onKey() {
            var theKey = event.code;
            if (theKey == 'Escape' && this.active)
