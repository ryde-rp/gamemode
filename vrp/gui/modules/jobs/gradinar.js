const gradinar = new Vue({
    el: ".gradinar-container",
    data: {
        active: false,
        userSkill: 1,
        selectedSkill: 1,
    ...
    methods: {
        ...
        build(data) {
            ...
            $(".gradinar-container").fadeIn();
            $(".jobs-container").fadeIn();
        },
        destroy() {
            if (!this.active) return;
            this.active = false;
            $(".gradinar-container").fadeOut();
            $(".jobs-container").fadeOut();
            this.post('jobExit', {exit: true})
        },
        startgradinar: function() {
            this.post('startJob', {job: 'Gradinar', skill: this.selectedSkill});
            this.destroy();
        },
        StopJob() {
            this.post('quitJob', {job: 'Gradinar'});
            this.destroy();
        },
