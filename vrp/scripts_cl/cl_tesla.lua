...
local vehicles = {"gcmetronsportback2021","gcmmodelsplaid2021","xplaid24"} -- define what cars can be used as a tesla (by modelname), set to "" (empty) for all cars
local debug = false
...
local function minimap(text)
	SetNotificationTextEntry("STRING")
	AddTextComponentString(text)
	DrawNotification(0,1)
end

local findTesla = function()
    local ped = PlayerPedId()
    local ped_veh = GetVehiclePedIsIn(ped)
    if ped_veh ~= 0 then
        local veh_model = GetEntityModel(ped_veh)
        for k,v in pairs(vehicles) do
            if veh_model == GetHashKey(v) then
                return true
            end
        end
    end
...
RegisterCommand("tesla", function(source, args)
	if(args[1] == "mark") then
		if(IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and (findTesla() or not next(vehicles))) and DecorGetInt(GetVehiclePedIsIn(PlayerPedId()), "FairPlay_Owner") == LocalPlayer.state.user_id then
			tesla = GetVehiclePedIsIn(GetPlayerPed(-1), false)
			SetEntityAsMissionEntity(tesla, true, true)
			minimap("Acest vehicul a fost marcat ca fiind tesla ta.")
      ...
      AddTextComponentString("Tesla")
			...
			tesla = nil
			minimap("Tesla ta a fost nemarcata.")
			if(DoesBlipExist(tesla_blip)) then
				RemoveBlip(tesla_blip)
			end
			tesla_blip = nil
		...
		if(IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and ((findTesla() or not next(vehicles)))) then
			if(args[1] == "pilot") then
				waypoint = Citizen.InvokeNative(0xFA7C7F0AADF25D09, GetFirstBlipInfoId(8), Citizen.ResultAsVector())
				if(IsWaypointActive()) then
					if(pilot) then
						pilot = false
						minimap("Auto-Pilot anulat.")
						ClearPedTasks(GetPlayerPed(-1))
					else
						if(crash) then
							crash = false
							minimap("Anti-Crash oprit.")
						end
						pilot = true
						minimap("Auto-Pilot activat.")
			...
			elseif(args[1] == "crash") then
				if(crash) then
					crash = false
					minimap("Anti-Crash dezactivat.")
				elseif(pilot) then
					pilot = false
					ClearPedTasks(GetPlayerPed(-1))
					minimap("Auto-Pilot dezactivat.")
					crash = true
					minimap("Anti-Crash activat.")
				else
			...
			elseif(args[1] == "lines") then
				if(lines) then
					minimap("Linii ajutatoare dezactivate.")
					lines = false
				else
					minimap("Linii ajutatoare activate.")
					lines = true
					Citizen.CreateThread(function()
						while lines do
							Wait(5)
				...
				if(tesla_pilot) then
					...
					Citizen.CreateThread(function()
						while tesla_pilot do
							Wait(100)
							local dist = GetDistanceBetweenCoords(GetEntityCoords(tesla)["x"], GetEntityCoords(tesla)["y"], GetEntityCoords(tesla)["z"], player_coords.x, player_coords.y, player_coords.z, 0) 
							if(dist < 40.0) then
								-- SetVehicleForwardSpeed(tesla, 20.0)
								while GetEntitySpeed(tesla) - 10.0 > 10.0 do
									SetVehicleForwardSpeed(tesla, GetEntitySpeed(tesla) - 10.0)
									Wait(100)
								end
							end
							if(dist < 10.0) then
								while GetEntitySpeed(tesla) - (1.0 + dist/3) > 0.0 do
									SetVehicleForwardSpeed(tesla, GetEntitySpeed(tesla) - (1.0 + dist/3))
									Wait(100)
								end
								tesla_pilot = false
								--DeletePed(tesla_pilot_ped)
								...
								tesla_pilot_ped = nil
								SetVehicleEngineOn(tesla, false, false, false)
								SetVehicleForwardSpeed(tesla, 0.0)
								minimap("Auto-Pilot dezactivat.")
							end
						end
					end)
