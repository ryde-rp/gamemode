var Lumberjack = new Vue({
    ...
    methods:{
        StartLumberjack: function() {
            this.Post("startLumberjack", {skill: this.selectedSkill});
            $(".lumberjack-container").fadeOut();
        ...
        onKey() {
            ...
            if(theKey == "Escape") {
                $(".lumberjack-container").fadeOut();
                $(".jobs-container").fadeOut();
                this.Post("CloseLumberjack", {close: true});
            }
        },
        SelectSkil(skil) {
        ...
        onMessage() {
            let data = event.data;
            if (data.action == "OpenLumberjack") {
                $(".jobs-container").fadeIn();
                $(".lumberjack-container").fadeIn();
                this.UpdateSkills(data.skill);
                this.jobActive = data.jobActive;
            }
        },
        StopJob() {
            this.Post("job:quit", {job: "lumberjack"});
            $(".lumberjack-container").fadeOut();
            $(".jobs-container").fadeOut();
        },
